...
Future<String> britamDirx() async {
  ...
  if (Platform.isMacOS) {
    directory = Directory(
        '/Users/${Platform.environment['USER']}/Documents/Verselib');
  } else if (Platform.isWindows) {
    directory = Directory(
        '${Platform.environment['USERPROFILE']}\\Documents\\Verselib');
  } else if (Platform.isLinux) {
    directory = Directory(
        '/home/${Platform.environment['USER']}/Documents/Verselib');
  } else if (Platform.isAndroid) {
    directory = Directory('/storage/emulated/0/Documents/Verselib');
  }
  if (await directory.exists()) {
    //if folder already exists return path
    return directory.path;
