...
class _SavingScreenBodyState extends State<SavingBody> {
  ...
  void initState() {
    ...
    _bloc.add(const SavingVersesFetch());
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<SavingBloc, SavingState>(
      ...
      builder: (context, state) {
        return Scaffold(
          backgroundColor:
              state.status == Status.versesSaving ? Colors.black : Colors.white,
          body: SafeArea(
            child: Stack(
              children: [
  ...
  void _handleStateChanges(
    ...
    if (state.status == Status.versesFetched) {
    ...
    if (state.status == Status.versesSaved) {
  ...
  Widget _buildContent(SavingState state, Size size, double radius) {
    ...
    if (state.status == Status.versesSaving && state.verses.isNotEmpty) {
      return Stack(
        children: [
  ...
  Widget _buildErrorState(SavingState state, AppLocalizations l10n) {
    return EmptyState(
      ...
      showRetry: true,
      onRetry: () => _bloc.add(const SavingVersesFetch()),
    );
  }
}
